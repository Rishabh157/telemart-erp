export class NumberToWordsConverter {
    ...
    private static thousands: string[] = ['', 'Thousand', 'Million', 'Billion']
    ...
    public static convert(num: number): string {
        ...
        if (num < 100)
            return `${this.tens[Math.floor(num / 10)]} ${
                num % 10 > 0 ? this.underTwenty[(num % 10) - 1] : ''
            }`.trim()
        if (num < 1000)
            return `${this.underTwenty[Math.floor(num / 100) - 1]} Hundred ${
                num % 100 !== 0 ? this.convert(num % 100) : ''
            }`.trim()
            ...
            if (num < divisor) {
                ...
                const suffix =
                    num % Math.pow(1000, i) !== 0
                        ? this.convert(num % Math.pow(1000, i))
                        : ''
                const space = suffix ? ' ' : ''
