...
import { getHeight } from "../../molecules/MOLFilterChip/MOLFilterChip";
import {
  addDays,
  addMonths,
  endOfMonth,
...
const ATMDateFilterChip = ({
  ...
  return (
    <>
      <div
        className={`flex gap-2 shadow pl-2 border border-dashed rounded-md border-gray-300 w-fit bg-white hover:bg-slate-50 items-center cursor-pointer pr-1.5 ${getHeight("small")}`}>
        <div
        ...
        >
          ...
          <div>
            {dateFilterKeys?.find((filterKey) => filterKey?.isSelected() === true)?.label}
          </div>
        </div>
                ...
                {dateFilterKeys?.map((filterKey, index) => {
                  ...
                  return (
                    <li
                      ...
                      className={`px-4 py-1 cursor-pointer ${isSelected && "bg-gray-200"} rounded`}>
                      {filterKey.label}
                    </li>
                  );
                })}
            ...
            >
              ...
              <div className="flex gap-2 p-2 border-b">
                <div
                  ...
                  className={`text-xs cursor-pointer text-slate-700 font-semibold px-2 py-1 rounded transition-all duration-300 ${pickingFor === "StartDate" && "bg-primary-main text-white"}`}>
                  Start Date
                </div>
                <div
                  onClick={() => setPickingFor("EndDate")}
                  className={`text-xs cursor-pointer text-slate-700 font-semibold px-2 py-1 rounded transition-all duration-300 ${pickingFor === "EndDate" && "bg-primary-main text-white"}`}>
                  End Date
                </div>
              </div>
                ...
                <ul className="p-2 border-r">
                  {quickPickerOptions?.map((option, index) => {
                    ...
                    return (
                      <li
                        key={index}
                        className={`cursor-pointer px-2 py-1.5 rounded-md text-xs hover:bg-primary-main hover:text-white text-slate-700 transition-colors duration-300 font-medium ${isSelected && "bg-primary-main text-white"}`}
                        onClick={() => {
                ...
                <div>
                  {pickingFor === "StartDate" ? (
                    <ReactDatePicker
                      ...
                      startDate={startDate ? new Date(startDate) : null as any}
