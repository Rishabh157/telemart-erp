...
const ProductCategoryListingWrapper = () => {
        ...
        <div className="relative">
          ...
          {showDropdown && currentId === row?._id && (
            <div className="absolute top-8 right-0 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              <button
                onClick={() => {
                  navigate(`/configurations/product-category/${currentId}`);
                }}
  ...
  const handleDelete = () => {
    ...
    deleteProductCategory(currentId).then((res: any) => {
      if ("data" in res) {
        if (res?.data?.status) {
          showToast("success", "product-category deleted successfully!");
        } else {
  ...
  return (
    <>
      <ConfigurationLayout>
        <ProductCategoryListing columns={columns} rows={items} setShowDropdown={setShowDropdown} />
      </ConfigurationLayout>
    </>
  );
};
