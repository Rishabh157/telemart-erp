...
type BarcodeCardProps = {
  cardBoxBarcodeList: CartonBoxBarcodeListResponse[];
...
const CartonBoxBarcodeDetailCard = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3  gap-5 py-3 px-3">
      {cardBoxBarcodeList.map((barcode: CartonBoxBarcodeListResponse, barcodeIndex: number) => {
        ...
        return (
          ...
          >
            {/*Checkbox */}
            <button
              onClick={(e) => onCartonBoxBarcodeSelect(e, barcode, isBarcodeSeleted)}
              className={`
                  flex 
                  justify-center 
                  items-center 
