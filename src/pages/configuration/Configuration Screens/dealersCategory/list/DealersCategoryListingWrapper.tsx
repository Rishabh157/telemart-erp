...
const DealersCategoryListingWrapper = () => {
        ...
        <div className="relative">
          ...
          {showDropdown && currentId === row?._id && (
            <div className="absolute top-8 right-0 bg-white border border-gray-200 rounded-md shadow-lg z-10">
              <button
                onClick={() => {
                  navigate(`/configurations/dealers-category/${currentId}`);
                }}
  ...
  const handleDelete = () => {
    ...
    deleteDealersCategory(currentId).then((res) => {
      if ("data" in res) {
        if (res?.data?.status) {
          showToast("success", "Dealers category deleted successfully!");
        } else {
  ...
  return (
    <>
      <ConfigurationLayout>
        <DealersCategoryListing columns={columns} rows={items} />
      </ConfigurationLayout>
    </>
  );
};
