...
// |-- Redux --|
import UnAssignBatchesOrderListing from './UnAssignBatchesOrderListing'
...
const UnAssigneBatchOrderListingWrapper = () => {
    useUnmountCleanup()
    ...
    const inboundCallerState: any = useSelector((state: RootState) => state.listingPagination)
    ...
    const { items } = useGetCustomListingData({
        useEndPointHook: useGetOrderQuery({
            ...
            filterBy: [
                ...
                {
                    fieldName: 'status',
                    value: [OrderStatusEnum.FRESH, OrderStatusEnum.URGENT, OrderStatusEnum.PREPAID],
                },
                {
    ...
    const columns: columnTypes[] = [
        ...
        //     extraClasses: 'text-xs min-w-[150px]',
        //     renderCell: (row: OrderListResponse) => (
        //         <span className='font-bold text-primary-hover'>{row.batchNumber}</span>
        //     ),
        // },
        {
            field: 'batchNumber',
            ...
            flex: 'flex-[1_1_0%]',
            name: UserModuleNameTypes.ASSIGN_BATCH_LIST_ORDER_NUMBER,
            extraClasses: 'text-xs min-w-[150px]', // Slightly larger font size
            renderCell: (row: OrderListResponse) => (
                row.batchNumber ? (<div className="bg-primary-main text-white px-2 py-1 rounded-md text-center font-bold">
                    {row.batchNumber}
                </div>) : null
            ),
        },
...
export default UnAssigneBatchOrderListingWrapper
