...
import { useGetSchemeByIdQuery, useUpdateSchemeMutation } from "src/services/SchemeService";
...
const EditSchemeWrapper = () => {
 ...
 const { userData } = useSelector((state: RootState) => state?.auth);
  ...
  const {selectedItem}:any=useSelector((state:RootState)=>state.scheme)
  const [UpdateScheme]=useUpdateSchemeMutation()
  const {data:updateData , isLoading:updateisLoading ,isFetching:updateisFetching}=useGetSchemeByIdQuery(Id)
  ...
  const initialValues: FormInitialValues = {
    ...
    productInformation:selectedItem?.productInformation?.map((ele:any)=>{
      return {
  ...
  const onSubmitHandler = (values: FormInitialValues) => {
    if (activeStep === steps?.length - 1) {
      ...
      const productInformationData=values.productInformation?.map((ele:any)=>{
        const {_id , ...rest}=ele;
        return rest
      })
